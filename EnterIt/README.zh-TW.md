# EnterIt

[Greasy Fork](https://greasyfork.org/zh-TW/scripts/565107-enterit) | [GitHub](https://github.com/clover-yan/userscripts/tree/master/EnterIt)

---

這不是一段冰冷的代碼，而是為了終結那份「按下 Enter 就會失去一切」的恐懼，所譜寫的救贖樂章。

這是一個 Tampermonkey 使用者腳本。當我們與 AI 相互對峙時，那些輕易被發送出去的殘缺語句，難道不就像是無法傳達的心意嗎？為了守護對話的完整性，EnterIt 選擇了改寫這份命運的劇本。

本腳本的靈魂碎片傳承自 [masachika-kamada/ChatGPT-Ctrl-Enter-Sender](https://github.com/masachika-kamada/ChatGPT-Ctrl-Enter-Sender) 的意志。

## 命運的變奏

- `Enter` 不再是終結，而是如詩歌般的「換行」。在對話框中自由地呼吸，留下屬於你的空白。
- `Ctrl+Enter`（⌘⏎）才是真正的「傳送」。唯有在指尖重重按下的那一刻，這份覺悟才會傳遞到彼岸。

## 演出的舞台

標註 \* 的舞台，是已經過親手確認、確信能產生共鳴的領域。

- [ChatGPT](https://chatgpt.com) \*
- [Claude](https://claude.ai)
- [Gemini](https://gemini.google.com) \*
- [Perplexity](https://www.perplexity.ai)
- [DeepSeek](https://chat.deepseek.com) \*
- ~~[Grok](https://grok.com) \*~~ — 不再支援：Grok 設定裡已經提供了該功能
- [GitHub Copilot](https://github.com/copilot) \*
- [NotebookLM](https://notebooklm.google.com) \*
- [Phind](https://www.phind.com)
- [Poe](https://poe.com)
- [Mistral Chat](https://chat.mistral.ai)
- [You.com](https://you.com)
- [v0](https://v0.dev)
- [Copilot](https://copilot.microsoft.com) \*
- [Microsoft 365 Chat](https://m365.cloud.microsoft) \*
- [元寶](https://yuanbao.tencent.com) \*
- [豆包](https://www.doubao.com) \*
- [千問](https://www.qianwen.com) (大陸版) \*
- [Qwen](https://chat.qwen.ai) \*
- [文心一言](https://yiyan.baidu.com) \*

*即使世界再怎麼混亂，至少這份輸入的節奏，要由我們自己來譜寫。*
